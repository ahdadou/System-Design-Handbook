  ...
  return (
    <div className="space-y-6 text-[#94a3b8]">
      <p className="text-base leading-relaxed">
        <strong className="text-[#f1f5f9]">Relational databases (RDBMS)</strong> organize data into tables of rows and columns with strict, predefined schemas. SQL (Structured Query Language) provides a declarative way to query, insert, update, and delete data. PostgreSQL, MySQL, and SQLite are among the most widely deployed software systems in the world.
      </p>
      <p className="text-sm leading-relaxed">
        The relational model was introduced by Edgar Codd in 1970. Its enduring power comes from two things: <strong className="text-[#f1f5f9]">expressive JOIN operations</strong> that let you combine data across tables without duplicating it, and <strong className="text-[#f1f5f9]">ACID guarantees</strong> that make transactional consistency predictable.
      </p>
      ...
      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">ACID Guarantees</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {[
          ...
          <div key={a.letter} className="p-4 rounded-xl border border-[#1e293b]" style={{ backgroundColor: `${a.color}10` }}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xl font-black font-heading" style={{ color: a.color }}>{a.letter}</span>
              <span className="font-bold text-sm font-heading text-[#f1f5f9]">{a.word}</span>
            </div>
      ...
      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">The N+1 Query Problem</h2>
      <p className="text-sm leading-relaxed">
...
for (const post of posts) {
  post.author = await User.findById(post.userId); // N queries
      ...
      <div className="p-4 rounded-xl bg-[#10b981]/10 border border-[#10b981]/30 mt-2">
        <div className="font-bold text-[#10b981] text-sm font-heading mb-2">Good Pattern (JOIN / Eager Load)</div>
        <pre className="text-xs text-[#94a3b8] whitespace-pre-wrap font-mono">
{`const posts = await Post.findAll({
  include: [{ model: User, as: "author" }]  // 1 JOIN query
      ...
      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">Materialized Views</h2>
      <p className="text-sm leading-relaxed">
        A <strong className="text-[#f1f5f9]">materialized view</strong> is a computed view whose results are stored as a real table on disk. Instead of re-running an expensive aggregation query on every request, you pre-compute it and read from the cached result.
      </p>
      <div className="space-y-3 mt-3">
        {[
          ...
          <div key={m.label} className="p-3 rounded-lg border border-[#1e293b] bg-[#111827] flex gap-3">
            <div className="w-2 rounded-full shrink-0" style={{ backgroundColor: m.color }} />
            <div>
              <div className="font-semibold text-sm text-[#f1f5f9] font-heading">{m.label}</div>
      ...
      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">Query Optimization Tips</h2>
