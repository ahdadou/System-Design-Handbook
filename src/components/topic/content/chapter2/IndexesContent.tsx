  ...
  return (
    <div className="space-y-6 text-[#94a3b8]">
      <p className="text-base leading-relaxed">
        A <strong className="text-[#f1f5f9]">database index</strong> is a data structure that improves the speed of data retrieval operations at the cost of additional storage and slower writes. Without indexes, every query would require a full table scan — reading every row to find matches.
      </p>

      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">How Indexes Work</h2>
      <p className="text-sm">Most databases use <strong className="text-[#f1f5f9]">B-tree indexes</strong> (balanced binary search tree). They maintain sorted copies of column values pointing to row locations. A B-tree lookup is O(log n) — searching 1 billion rows takes only ~30 comparisons.</p>

      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">Dense vs Sparse Indexes</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="p-4 rounded-xl bg-[#3b82f6]/10 border border-[#3b82f6]/30">
          <div className="font-bold text-[#3b82f6] text-sm font-heading mb-2">Dense Index</div>
          <p className="text-xs text-[#94a3b8]">An entry for every single row in the table. Direct pointer to each row. Faster lookups but uses more storage.</p>
          <p className="text-xs text-[#475569] mt-2">MySQL InnoDB secondary indexes, most B-tree indexes</p>
        </div>
        <div className="p-4 rounded-xl bg-[#10b981]/10 border border-[#10b981]/30">
          <div className="font-bold text-[#10b981] text-sm font-heading mb-2">Sparse Index</div>
          <p className="text-xs text-[#94a3b8]">Only entries for some rows (e.g., first row of each disk page). Smaller index but requires scanning after finding the page.</p>
          <p className="text-xs text-[#475569] mt-2">PostgreSQL heap indexes, clustered primary keys</p>
        </div>
      </div>
      ...
      <h2 className="text-2xl font-bold font-heading text-[#f1f5f9]">Index Best Practices</h2>
      <ul className="space-y-2 text-sm list-none">
        {[
          ...
          <li key={i} className="flex items-start gap-2"><span className="text-[#3b82f6] mt-1 shrink-0">→</span><span>{item}</span></li>
        ))}
      </ul>
